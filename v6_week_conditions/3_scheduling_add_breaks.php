...
// Расширим алгоритм, теперь нужно добавить перерывы для ВСЕХ видов смен
...
// ВАЖНО! В старом Verint WFM можно задавать начала перерывов неявно при помощи min/max spacing
// По крайней мере поначалу этим неявным условием пользоваться не будем
// Вычисляем из явного условия - это гораздо проще

// Начальные точки перерывов взяты из настроек смен - запросил Маргариту переписать в явном виде
// На правило min/max spacing будем проверять по факту составления расписания и выводить предупреждения
...
// При заполнении массивов начал перерывов можно воспользоваться тем же массивом конвертации
// Здесь нужно помечать уникальными индексами виды перерывов, т.к. перерывы все уникальные
// В реальном коде имеет смысл помечать id-шником записи shiftEvent
// Также как и смену имеет смысл помечать не порядковым номером с нуля, а id-шником, чтоб не запустаться
...
// Перерывы для 2106_0.5_ENV
$startBreakPoints[0] = [
    2 => [
        1 => [ // 1 - это 15 мин
            $convertionArray['01:30'],
            $convertionArray['01:45'],
            $convertionArray['02:00'],
            $convertionArray['02:15'],
            $convertionArray['02:30'],
        ],
    ],
];

// Перерывы для 2106_0.75
$startBreakPoints[1] = [
    3 => [
        1 => [
            $convertionArray['01:30'],
    ...
    4 => [
        1 => [
            $convertionArray['03:45'],
            $convertionArray['04:00'],
            $convertionArray['04:15'],
            $convertionArray['04:30'],
            $convertionArray['04:45'],
            $convertionArray['05:00'],
            $convertionArray['05:15'],
        ],
    ],
];

// Перерывы для 2106_0.5_MRN_DAY
$startBreakPoints[2] = [
    5 => [
        1 => [
            $convertionArray['01:30'],
...
$startBreakPoints[3] = [
    ...
    7 => [
        2 => [ // 2 - это 30 мин
            $convertionArray['03:15'],
            $convertionArray['03:30'],
            $convertionArray['03:45'],
            $convertionArray['04:00'],
            $convertionArray['04:15'],
            $convertionArray['04:30'],
    ...
    8 => [
        1 => [ // 1 - это 15 мин
            $convertionArray['06:30'],
            $convertionArray['06:45'],
            $convertionArray['07:00'],
            $convertionArray['07:15'],
        ],
    ],
];

// Перерывы для 2106_0.5_старт 17-18
$startBreakPoints[4] = [
    9 => [
        1 => [ // 1 - это 15 мин
            $convertionArray['01:30'],
...
// Перерывы для 2106_0.5_старт 10-12
$startBreakPoints[5] = [
    10 => [
        1 => [ // 1 - это 15 мин
            $convertionArray['01:30'],
...
// Перерывы для 2106_0.5_старт 14-16
$startBreakPoints[6] = [
    11 => [
        1 => [ // 1 - это 15 мин
            $convertionArray['01:30'],
...
// Перерывы для 2106_0.75_старт 8-9
$startBreakPoints[7] = [
    12 => [
        1 => [ // 1 - это 15 мин
            $convertionArray['01:30'],
...
// Перерывы для 2106_0.75_старт 14:30-15:30
$startBreakPoints[8] = [
    14 => [
        1 => [ // 1 - это 15 мин
            $convertionArray['01:30'],
...
// Перерывы для 2106_0.75_старт 12-15
$startBreakPoints[9] = [
    16 => [
        1 => [ // 1 - это 15 мин
            $convertionArray['01:30'],
...
// Перерывы для 2106_0.75_старт 10-12
$startBreakPoints[10] = [
    18 => [
        1 => [ // 1 - это 15 мин
            $convertionArray['01:30'],
